 ...
 * @author MHD Balaj Khan
 */
let $progressIcon = document.querySelector(".progress-icon path");
let $progressIconLenght = $progressIcon.getTotalLength();

const scrollTopFunction = () => {
    let $scrollTop = $(window).scrollTop();
    let $getHeight = $(document).height() - $(window).height();
    $progressIcon.style.strokeDashoffset = $progressIconLenght - ($scrollTop * $progressIconLenght) / $getHeight;
}

scrollTopFunction();
$(window).scroll(scrollTopFunction);

const scrollTopCondition = (x) => {
    50 < $(x).scrollTop() ? $(".progress-icon").addClass("isActive") : $(".progress-icon").removeClass("isActive");
}

$(window).on("scroll", (event) => {
    scrollTopCondition(event.target);
}),

scrollTopCondition(window);

// 50 < $(window).scrollTop() ? $(".progress-icon").addClass("isActive") : $(".progress-icon").removeClass("isActive");

$(".progress-icon").on("click", () => $("html, body").animate({ scrollTop: 0 }, 550) );
...
// $('.scroll-animation a').click(function(){
//   $('html, body').animate({
...
$(document).on('click', '.scroll-animation a', function() {
  $('html, body').animate({
    scrollTop: $($(this).attr('href')).offset().top -100
  }, 500);
  return false;
